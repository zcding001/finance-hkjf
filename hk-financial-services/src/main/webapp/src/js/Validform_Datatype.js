...
(function(){
	if($.Datatype){
		$.extend($.Tipmsg.w,{
			...
			"idcard":"身份证号码不对！",
			"N2":"请填写最大是两位小数的数字！",
			"loginName":"新密码不能与用户名相同！",
			"logName":"用户名不一致，请重新输入！",
			"mobile":"请输入正确的手机号码！",
			"logTel":"原手机号码不一致，请重新输入！"
		});
		...
		$.extend($.Datatype,{
			...
			*/
			...
			"N2":/^(\d{1,8}|\d{1,8}\.\d{1,2})$/,
			"mobile":/^0{0,1}(13[0-9]|15[0-9]|18[0-9]|14[0-9]|16[0-9]|17[0-9])[0-9]{8}$/,
			
			//TIME
			"time":/^([01]\d|2[0123]):([0-5]\d):([0-5]\d)$/,
			/*
			  参数gets是获取到的表单元素值，
			  obj为当前表单元素，
			  curform为当前验证的表单，
			...
			*/
			...
			"num_max":function(gets,obj,curform,datatype){
				/*
					判断数值范围
					注意需要给表单元素绑定max,min属性来指定最大或最小可输入的值，没有绑定的话使用默认值
				*/
				...
				var maxim=~~obj.attr("max") || 100000000;
				var	minim=~~obj.attr("min") || 0;
				if(parseFloat(gets)>parseFloat(maxim)){
					return "输入的数字最大为"+maxim+"！";
				}
				if(parseFloat(gets)<parseFloat(minim)){
					return "输入的数字最小为"+minim+"！";
				}
				/*
					当前值不能与指定表单元素的值一样，如新密码不能与旧密码一样，密码不能设置为用户名等
					注意需要通过绑定with属性来指定要比较的表单元素，可以是clas，id或者是name属性值
				...
				*/
				var withele=$.trim(obj.attr("with"));
				var val=curform.find(withele+",[name='"+withele+"']").val() || withele;
				if(parseFloat(gets)>parseFloat(val)){
					return "输入的数字不能大于"+val+"！";
				}else{
					return true;
				}
			},
			"num_min":function(gets,obj,curform,datatype){
				/*
					判断数值范围
					注意需要给表单元素绑定max,min属性来指定最大或最小可输入的值，没有绑定的话使用默认值
				*/
				
				var maxim=~~obj.attr("max") || 99999999;
				var	minim=~~obj.attr("min") || 0;
				if(parseFloat(gets)>parseFloat(maxim)){
					return "输入的数字最大为"+maxim+"！";
				}
				...
				*/
				...
				var val=curform.find(withele+",[name='"+withele+"']").val() || withele;
				if(parseFloat(gets)<parseFloat(val)){
					return "输入的数字不能小于"+val+"！";
				}else{
					return true;
				}
			},
			"password":function(gets,obj,curform,datatype){
				/*
					判断数值范围
					注意需要给表单元素绑定max,min属性来指定最大或最小可输入的值，没有绑定的话使用默认值
				*/
				var expr = new RegExp(/^(?![0-9]+$)(?![a-zA-Z]+$)[0-9A-Za-z]{6,16}$/);
				//var expr = new RegExp(/^[\S]{6,16}$/);
				var nullMsg=~~obj.attr("nullMsg") || "请输入信息！";
				var errorMsg=~~obj.attr("errorMsg") ||"请输入6至16位字母数字组合!";
				if(!expr.test(gets)){
					return errorMsg;
				}
				if(gets==''){
			...
			"pwdAuthent":function(gets,obj,curform,datatype){
				/*
					判断数值范围
					注意需要给表单元素绑定max,min属性来指定最大或最小可输入的值，没有绑定的话使用默认值
				*/
				var expr = new RegExp(/^[\S]{6,16}$/);
				var spwd = obj.attr("reck");
				var repwd = $("#"+spwd).val();
				var nullMsg=~~obj.attr("nullMsg") || "请输入信息！";
			...
			"loginName":function(gets,obj,curform,datatype){
				var withele=$.trim(obj.attr("loginName"));
				var val=curform.find(withele+",[name='"+withele+"']").val() || withele;
				if(gets==$.trim(val)){
					return false;
				}
			...
			"logName":function(gets,obj,curform,datatype){
				var withele=$.trim(obj.attr("loginName"));
				var val=curform.find(withele+",[name='"+withele+"']").val() || withele;
				if(gets!=$.trim(val)){
					return false;
			...
			"logTel":function(gets,obj,curform,datatype){
				var withele=$.trim(obj.attr("logTel"));
				var val=curform.find(withele+",[name='"+withele+"']").val() || withele;
				if(gets!=$.trim(val)){
					return false;
				}else{
