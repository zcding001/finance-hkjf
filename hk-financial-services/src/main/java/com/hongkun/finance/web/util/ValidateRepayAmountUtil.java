...
public class ValidateRepayAmountUtil {
	...
	public static ResponseEntity<?> normalRepayMoney(BidInfoDetail bidInfoDetail, BigDecimal punishAmount,
		...
		// 正常还款总金额
		BigDecimal repayAmountSum = repayAmount.add(punishAmount);
        ...
        // 不预留利息，还款金额直接从可用金额扣除
		if (bidInfoDetail.getReserveInterest() == InvestConstants.BID_DETAIL_RESERVE_INTEREST_NO && CompareUtil.gt(repayAmountSum, useableMoney)) {
			buffer.append("借款人可用金额不足，剩余可用金额：").append(useableMoney);
			return BaseUtil.error(buffer.toString());
		}
		if (bidInfoDetail.getReserveInterest() == InvestConstants.BID_DETAIL_RESERVE_INTEREST_YES && CompareUtil.gt(repayAmountSum, nowMoney)) {
	...
	public static ResponseEntity<?> advanceRepayMoney(BidInfo bidInfo, BidInfoDetail bidInfoDetail,
		...
		// 利息
		BigDecimal calInterest = RepayCalcInterestUtil.calInterestForReturnCap(bidInfo, bidInfoDetail, capital,
				repayPlan);
		repayAmount.add(calInterest);
		// 服务费
		if (CompareUtil.gtZero(bidInfo.getServiceRate())) {
			BigDecimal calService = RepayCalcInterestUtil.calInterestForReturnCap(bidInfo, bidInfoDetail, capital,
					repayPlan);
			repayAmount.add(calService);
		}
		return normalRepayMoney(bidInfoDetail, BigDecimal.ZERO, repayAmount, account);
