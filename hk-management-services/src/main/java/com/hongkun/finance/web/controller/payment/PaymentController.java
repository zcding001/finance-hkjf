...
public class PaymentController {
	...
	public ResponseEntity<?> paymentChannelState(Integer id, Integer state) {
		try {
		    FinChannelGrant channelGrant = this.finChannelGrantService.findFinChannelGrantById(id);
			if(channelGrant != null){
			   if(PayStyleEnum.WITHDRAW.getValue() == channelGrant.getPayStyle()){
			       FinChannelGrant withdrawChannelGrant = new FinChannelGrant();
			       withdrawChannelGrant.setState(TradeStateConstants.START_USING_STATE);
			       withdrawChannelGrant.setPayStyle(PayStyleEnum.WITHDRAW.getValue());
			       List<FinChannelGrant>  channelGrantList = this.finChannelGrantService.findFinChannelGrantList(withdrawChannelGrant);
			       List<FinChannelGrant> channelList = new ArrayList<FinChannelGrant>();
			       if(channelGrantList!=null  && channelGrantList.size()>0){
			         channelGrantList.forEach(e->{
			             FinChannelGrant channel =new FinChannelGrant();
			             channel.setId(e.getId());
			             channel.setState(TradeStateConstants.FORBIDDEN_STATE);
			             channelList.add(channel);
			         });
			         this.finChannelGrantService.updateFinChannelGrantBatch(channelList, channelList.size());
			       }
			   }
			   ...
			   this.finChannelGrantService.updateFinChannelGrant(finChannelGrant);
			    ...
			    return new ResponseEntity<>(Constants.ERROR, "没有查询到对应的支付渠道，更新失败！");
			}
		} catch (Exception e) {
		    logger.error("更新支付模式状态异常, 支付模式id: {}, 异常信息: {}",id,e);
