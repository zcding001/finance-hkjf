...
app.controller('CommunityController', function ($scope, $timeout, $http, $CommonService, $compile, $location) {
    ...
    $scope.createAddCommunityPanal =function () {
        ...
        commonUtil.createCustomConfirmBox("添加小区/门店", contents, CONFIG.interface.saveCommunity,function () {
            if($("input[name=addType]").get(1).checked){
                if(commonUtil.isEmpty($("select[name=regUserId]").val()) || "none" == $("select[name=regUserId]").val()){
                    alert("请选择物业公司！");
                    return false;
                }
                if(commonUtil.isEmpty($("select[name=pid]").val()) || "none" == $("select[name=pid]").val()){
                    alert("请选择小区名称！");
                    return false;
                }
                if(commonUtil.isEmpty($("#tenementInput").val())){
                    alert("门店名称不能为空！");
                    return false;
                }
            }else if($("input[name=addType]").get(0).checked){
                if(commonUtil.isEmpty($("#communityNameInput").val())){
                    alert("小区名称不能为空！")
                    return false;
                }
            }else{
            ...
            return true;
        },commonUtil.customConfirmStandardAfterExecuteFn);
