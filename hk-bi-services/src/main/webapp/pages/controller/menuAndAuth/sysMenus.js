//维护系统菜单的controller
app.controller('sysMenus', function($scope,$http){

    //添加菜单前审核
    $scope.validateCheck = function () {
        if (commonUtil.isEmpty($("#menuNameArea").val())) {
        ...
        if ($("#pid").val() != 0 && commonUtil.isEmpty($("#menuUrlArea").val())) {
    ...
    $scope.validateAppCheck = function () {
        if (commonUtil.isEmpty($("#titleArea").val())) {
            $("#checkPacketErro").text('请填写菜单名称!');
            return false;
        }
        if (commonUtil.isEmpty($("#linkKeyArea").val())) {
            $("#checkPacketErro").text('请填链接Key!');
            return false;
        }
        ...
        if ($("#id").val()== 0) {
            if($("#imgArea")[0].files[0]==null||$("#imgArea")[0].files[0]==undefined){
    ...
    $scope.uploadAppMenu=function () {
        var url=CONFIG.interface.addAppMenu;
        if ($("#id").val()!= 0) {
            url=CONFIG.interface.updateAppMenu;
        }
        //执行上传
        var data = new FormData();
        data.append('title', $("#titleArea").val());
        data.append('linkKey', $("#linkKeyArea").val());
        data.append('seq', $("#seqArea").val());
        if ($("#id").val()!= 0) {
            data.append('id', $("#id").val());
            data.append('updateFile', $("#imgArea")[0].files[0]);
        }else{
            data.append('unSavedFile', $("#imgArea")[0].files[0]);
        }
        $http.post(url, data, {
    ...
    //创建或者修改APp端Menu的Panl
    $scope.createAddOrUpdateAppMenuPanel = function (id, title, menuUrl, linkKey, seq) {
        //隐藏其他的弹框
        $("#childMenusAlertModal").modal('hide');
        ...
        var contents = '';
        contents += '<input id="id"  name="id" type="hidden" value="' + id + '" checked="checked"/>';
        contents += '菜单名称：<input id="titleArea" name="title" type="text" value="' + title + '" class="confirmChildren"/> <br><br>';
        contents += 'LinkKey：<input id="linkKeyArea" name="linkKey" type="text" value="' + linkKey + '" class="confirmChildren"/> <br><br>';
        contents += '排序编号：<input id="seqArea" name="seq" type="text" value="' + seq + '" class="confirmChildren"/> <br><br>';
        contents += '请选择菜单图片：<input id="imgArea" name="unSavedFile" type="file" /> <br><br>';
        contents += '<span id="checkPacketErro" style="color: red"></span>'
